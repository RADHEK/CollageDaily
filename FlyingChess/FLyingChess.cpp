...
void Action(int r, int t)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        if (Move(plane[t][i] + r) >= 56)
        {
            plane[t][i] = 0;
            state[t][i] = 0;
            cout << "I made a choice to win" << endl;
    ...
    {
        if ((state[t][i] == 0 || plane[t][i] == 0) && Move(plane[t][i] + r) <= 50)
            continue;
        for (j = 1; j < 4; j++)     //number of other player
            for (k = 0; k < 4; k++) //number of other player's plane
                if (plane[(t + j) % 4][k] <= 50 && plane[(t + j) % 4][k] != 0 && ((Move(plane[t][i] + r) + 6 + t * 13) % 52 == (plane[(t + j) % 4][k] + 6 + ((t + j) % 4) * 13) % 52))
                {
                    plane[t][i] = Move(plane[t][i] + r);
                    cout << "I made a choice to crash" << color[(t + j) % 4] << " " << k + 1 << endl;
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
            plane[t][i] = Move(plane[t][i] + r);
            cout << "I made a choice to stay in track" << endl;
            cout << "Plane " << color[t] << " " << i + 1 << " moved." << endl;
            return;
        }
    }
    for (i = 0; i < 4; i++)
    {
        ...
        {
            plane[t][i] = Move(plane[t][i] + r);
            cout << "I made a choice to fly" << endl;
            cout << "Plane " << color[t] << " " << i + 1 << " moved." << endl;
            return;
        }
    }
    for (i = 0; i < 4; i++)
    {
        ...
        {
            plane[t][i] = Move(plane[t][i] + r);
            cout << "I made a choice to jump" << endl;
            cout << "Plane " << color[t] << " " << i + 1 << " moved." << endl;
            return;
        }
    }
    ...
    while ((!win) && turn < 10000)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            for (int j = 0; j < 4; j++)
                ...
                else if (plane[i][j] <= 50)
                    cout << (plane[i][j] + 6 + 13 * i) % 52 << " ";
                else
                    cout << plane[i][j]+2 << " ";
            cout << endl;
        }
